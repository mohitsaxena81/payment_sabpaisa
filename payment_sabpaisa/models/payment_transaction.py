...
class PaymentTransaction(models.Model):
    ...
    sabpaisa_txn_id = fields.Char()
    bank_txn_id = fields.Char()
    ...
    def create(self, values):
        # Fetch the related payment provider
        provider = self.env['payment.provider'].browse(values.get('provider_id'))
        if provider.code=='sabpaisa' and  not  (provider.auth_iv and provider.auth_key and
                 provider.pg_url and provider.client_code and
                 provider.client_username and provider.client_username):
          raise UserError(_("please set the all configuration fields"))
        # Check if the provider is configured


        # Call the super method to proceed with transaction creation
    ...
    def action_demo_set_done(self):
        ...
        """
        self.ensure_one()
        if self.provider_code != 'sabpaisa':
            return

        notification_data = {'reference': self.reference, 'simulated_state': 'done'}
        self._handle_notification_data('sabpaisa', notification_data)
    ...
    def _send_payment_request(self):
        ...
        print("done payment request...............")
        if self.provider_code != 'sabpaisa':
            return

        if not self.token_id:
            raise UserError("sabpaisa: " + _("The transaction is not linked to a token."))
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('sabpaisa', notification_data)
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        tx._handle_notification_data('sabpaisa', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'sabpaisa' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'sabpaisa')])
        if not tx:
            raise ValidationError(
                "sabpaisa: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'sabpaisa-{self.reference}'
